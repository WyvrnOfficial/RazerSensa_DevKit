...
Synesthesia 应用位于 `ReleaseConsole` 文件夹。控制台可用于QA或者其他测试。HapticFolders 也位于 Synesthesia 应用目录下的 `HapticFolders`，当已安装 Razer Chroma 时，该文件夹的最新版本可在 C:\Program Files (x86)\Interhaptics\HapticFolders 中查看。
...
##### 2.1.2 在 Chroma Sensa 集成游戏中测试触觉（示例：《霍格沃茨之遗》 / 《漫威争锋》）
...
- 插入 Razer Sensa 触觉设备。
- 确认《霍格沃茨之遗》/ 《漫威争锋》已启用为 Chroma App。
